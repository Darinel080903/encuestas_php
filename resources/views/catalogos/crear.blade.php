            ...
            <div class="col-md-12">
                <div class="card cardborde">
                    <div class="card-header justify-content-between align-items-centr text-center encabezadoform">
                        <h3 class="headerlistatitulo"><i class="fas fa-save"></i> Nuevo {{$vdescripcion ?? ''}}</h3>
                    </div>
                    
                    <div class="card-body">
                        
                        <form class="needs-validation" method="POST" action="{{ url('/catalogos') }}" novalidate>
                            @csrf

                            <div class="form-group">
                                <label for="parte">{{$vdescripcion ?? ''}}:</label>
                                <input type="text" class="form-control" name="descripcion" placeholder="descripcion" max="250" value="{{old('descripcion')}}" required/>
